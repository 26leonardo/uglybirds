...
public class Logic implements ILogic{
    ...
    public void setBirdIsFlying(boolean i){    //chiamalo set
        BIRD_IS_FLYING = i;
    }
        ...
        if(BIRD_IS_FLYING){
            ...
            for(int j=0; j<pigRect.length; j++){
                if(birdRect.intersects(pigRect[j])){
                    IS_EXPLODING=true;
                    explosionPoint1.x = pigRect[j].x;    //devi farlo valere per più di un esplosione in contemporanea
                    explosionPoint1.y = pigRect[j].y;    //devi farlo valere per più di un esplosione in contemporanea
                    pigRect[j].x=0;   
                    pigRect[j].y=0; 
                    updateScore(300);
                    if(countAlivePigs>0)
            ...
            for(int j=0; j<blockRect.length; j++){
                if(birdRect.intersects(blockRect[j])){
                    ...
                    if(explosionPoint1.x==0){
                        explosionPoint1.x = blockRect[j].x;    //devi farlo valere per più di un esplosione in contemporanea
                        explosionPoint1.y = blockRect[j].y;    //devi farlo valere per più di un esplosione in contemporanea
                    }else{
                        explosionPoint2.x = blockRect[j].x;    //devi farlo valere per più di un esplosione in contemporanea
                        explosionPoint2.y = blockRect[j].y;    //devi farlo valere per più di un esplosione in contemporanea
                    }
                    blockRect[j].x=0;
                    blockRect[j].y=0;
                    updateScore(150);
