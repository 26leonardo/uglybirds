            ...
            protected Void doInBackground() {
                while(true){
                    ...
                    if(gameEnded() && (!IS_EXPLODING)){  
                        rankings.readFile();
                        rankings.writeFile(getScore(), getPlayerName());         //serve controllo, se c'è già lo stesso nome con lo stesso unteggio non riscriverlo
                        IView.getIView().flip("END_MENU");
                        // if(!win)
                        //     SoundPlayer.getSoundPlayer().playGameOver();
                    }
                    try {
