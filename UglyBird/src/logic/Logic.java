...
//va bene che lascio array parzialmente "nullo"?
    ...
    public void updateBird(){  //poi devo mettere bene condizioni di quando vola, quando esplode etc
        if(BIRD_IS_FLYING){
            ...
            for(int j=0; j<pigRect.length; j++){
                if(birdRect.intersects(pigRect[j])){
                    ...
                    pigRect[j].x=MainGUI.WIDTH;   //lo disegno fuori, sarebbe meglio non disegnarlo proprio
                }
            }
