...
public class Canva extends JPanel implements ActionListener, MouseListener, MouseMotionListener{
    ...
    private static final int EXPLOSION_RATE=125/REFRESH_TIME;
...
SwingWorker<Void,Void> workero=new SwingWorker<Void,Void>() {
    @Override
    protected Void doInBackground() {
        while(true){
                                //se qui metto updateBird mi stampa due finestre
            repaint();
            try {
...
workero.execute();
    ...
    public void paintComponent(Graphics g){
        ...
        ILogic.getILogic().updateBird();
        g2.drawImage(Images.imagesArray[Images.RED_BIRD], ILogic.getILogic().getBirdPoint().x, ILogic.getILogic().getBirdPoint().y, null);
        
        if(Logic.IS_EXPLODING){ 
            this.drawExplosion(ILogic.getILogic().getExplosionPoint(), g2); //è decentrata
        }
    }
    ...
    public void drawPig(String s, Graphics2D g2){
        ...
        //     g2.drawImage(this.pig, ILogic.getILogic().getPigsPositions(ILogic.getILogic().getLevel())[i].x,         //devo passargli i rect, non le posizioni base
        ...
        for(int i=0; i<ILogic.getILogic().getNumberOfPigs(ILogic.getILogic().getLevel()); i++){
            g2.drawImage(this.pig, ILogic.getILogic().getPigsRect()[i].x,         //devo passargli i rect, non le posizioni base
                ILogic.getILogic().getPigsRect()[i].y, null);      //metti posizioni in array diversi in base al livello, cicla for con una variabile numer of pigs, dipendente dal livello
        }
    }
    ...
    public void drawBird(Graphics2D g2){    //quelli fermi
    ...
    public void drawExplosion(Point p, Graphics2D g2){
        ...
        if(explosionRefreshCounter==0){
            ILogic.getILogic().isExploding(false);
        }
    }
    ...
    public void mouseReleased(MouseEvent e) {       //mi permette di lanciare l'uccellino anche quando è in volo, devo impedirlo
        if((e.getX()<MainGUI.CATAPULT_CENTER_X) && (e.getY()>MainGUI.CATAPULT_CENTER_Y)){
            ...
            yReleased = e.getY();    
            trajectoryIsVisible=false;  
            ILogic.getILogic().birdIsFlying(true);      
        }
    } 
