                ...
                for(int i=0; i<3; i++){
                    if(ILogic.getILogic().getScoreRankings()[i]!=0){
                        ...
                        if(ILogic.getILogic().getBirdRankings()[i].equals("red")){
                            g2.drawImage(Images.imagesArray[Images.RED_BIRD].getScaledInstance(40, 40, Image.SCALE_AREA_AVERAGING),  
                                (200 - 40 - metrics.stringWidth("  ")), 395+i*50, null);
                        }else if(ILogic.getILogic().getBirdRankings()[i].equals("yellow")){
                            g2.drawImage(Images.imagesArray[Images.YELLOW_BIRD].getScaledInstance(40, 40, Image.SCALE_AREA_AVERAGING), 
                                (200 - 40 - metrics.stringWidth("  ")), 395+i*50, null);
                        }
                        else if(ILogic.getILogic().getBirdRankings()[i].equals("blue")){
                            g2.drawImage(Images.imagesArray[Images.BLUE_BIRD].getScaledInstance(40, 40, Image.SCALE_AREA_AVERAGING), 
                                (200 - 40 - metrics.stringWidth("  ")), 395+i*50, null);
                        }
                        else{
                            g2.drawImage(Images.imagesArray[Images.WHITE_BIRD].getScaledInstance(40, 40, Image.SCALE_AREA_AVERAGING), 
                                (200 - 40 - metrics.stringWidth("  ")), 395+i*50, null);
                        }

                        if(ILogic.getILogic().getScoreRankings()[i]>=ONE_STAR){
                                g2.drawImage(Images.imagesArray[Images.YELLOW_STAR].getScaledInstance(40, 40, Image.SCALE_AREA_AVERAGING), 
                                    (200 + metrics.stringWidth("  ")+ maxLenghtName + 40  + metrics.stringWidth(""+ ILogic.getILogic().getScoreRankings()[i]) + 20),  395+i*50, null);
                            if(ILogic.getILogic().getScoreRankings()[i]>=TWO_STAR){
                                g2.drawImage(Images.imagesArray[Images.YELLOW_STAR].getScaledInstance(40, 40, Image.SCALE_AREA_AVERAGING), 
                                    (200 + metrics.stringWidth("  ")+ maxLenghtName + 40  + metrics.stringWidth(""+ ILogic.getILogic().getScoreRankings()[i]) + 20 + 40),  395+i*50, null);
                            if(ILogic.getILogic().getScoreRankings()[i]>=THREE_STAR)
                                g2.drawImage(Images.imagesArray[Images.YELLOW_STAR].getScaledInstance(40, 40, Image.SCALE_AREA_AVERAGING), 
                                    (200 + metrics.stringWidth("  ")+ maxLenghtName + 40  + metrics.stringWidth(""+ ILogic.getILogic().getScoreRankings()[i]) + 20 + 40 + 40),  395+i*50, null);
                            }
                        }

                        g2.drawString("" + ILogic.getILogic().getDiffRankings()[i], 
                            (200 + metrics.stringWidth("  ")+ maxLenghtName + 40  + metrics.stringWidth(""+ ILogic.getILogic().getScoreRankings()[i]) + 20 + 40 + 40 + 40 + metrics.stringWidth("  ")), 430+i*50);
                    }
                }
            }else{
