...
public class Canva extends JPanel implements MouseListener, MouseMotionListener{
    ...
    public void paintComponent(Graphics g){
        ...
        if(ILogic.getILogic().getIsExploding()){ 
            if(ILogic.getILogic().getExplosionPoint2().x!=0){   
    ...
    private void drawBird(String s, Graphics2D g2){    
        ...
        if(ILogic.getILogic().getNumAliveBirds()!=0)
            g2.drawImage(this.bird, ILogic.getILogic().getBirdPoint().x, ILogic.getILogic().getBirdPoint().y, null);
        
        for(int i=1; i<ILogic.getILogic().getNumAliveBirds(); i++){ 
            g2.drawImage(this.bird, ILogic.getILogic().getBirdsPositions(ILogic.getILogic().getLevel())[i].x, 
    ...
    private void drawExplosion(Point p, Graphics2D g2){
        int x = p.x-(MainGUI.EXPLOSION_SIZE-IView.BLOCK_SIZE)/2;  //to center the explosion
        int y = p.y-(MainGUI.EXPLOSION_SIZE-IView.BLOCK_SIZE)/2;
    ...
    private void drawDoubleExplosion(Point p1, Point p2, Graphics2D g2){     //draws a centered explosion between two items
        int x1 = (p1.x+p2.x)/2-(MainGUI.EXPLOSION_SIZE-IView.BLOCK_SIZE)/2;  
        int y1 = (p1.y+p2.y)/2-(MainGUI.EXPLOSION_SIZE-IView.BLOCK_SIZE)/2;    
